...
def concentration_ratio(Y, SE, q=1e-2):
    """The ratio between actual fully-formed NS concentration and originally estimated NS concentration"""
    #number of base pairs in a strand without defect
    N = 2 + 2*Y + SE
    qty = np.array([0]*10+[n*(1-q)**(n-10) * q for n in range(10,N)]+[N*(1-q)**(N-10)])
    #average length is qty.sum()
    return (1-q)**(N-SE-10) * N/qty.sum()
...
for Y, SE, C_NS, icolor in [(16, 4, 600, 2), (16, 4, 800, 1), (16, 4, 1000, 0), (16, 6, 500,6), (16, 6, 1000, 4), (16, 8, 1000,8), (32, 6, 400, 7), (32, 6, 500, 6)]:
    ...
    #elif Y==32:
    #    sY ='32'
    pSEdata = read_csv(f'../simulations/melting_SE{SE}/A{sY}-SE{SE}_{int(np.ceil(C_NS*3)):d}uM_complexes_concentration_melting-1.tsv', sep='\t').rename(columns={'# temperature':'T'})
    pSE = CubicSpline(pSEdata['T'], pSEdata['A+B+A+B']/(1.5*C_NS*1e-6))
    ...
    axs[1].plot(
        phi_rotating_assembled_only_crossover(
            pSE(mintandelta[good,0])*0.99**SE,
            pNS(mintandelta[good,0]),
            SE=SE, C_0=C_NS*1e-6*concentration_ratio(Y, SE), Y=Y, persistence=50
            ),
        mintandelta[good,1],
        ls='none', marker=marks[Y], color=color,
        #label=f'Y{Y}SE{SE} {C_NS: >4d} µM'
